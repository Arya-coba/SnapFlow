...
import { 
  ...
  ChevronRight, Loader2, Plus, Mic, Bot, Clock, ArrowRight
} from 'lucide-react';
import Card from '../components/ui/Card';
import { getSessionUser } from './Topbar';
...
export default function Dashboard() {
  ...
  const userName = getSessionUser()?.nama?.split(' ')[0] || 'Pengguna';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const resHistory = await fetch(`${API.historyDocs}?limit=4`);
  ...
  return (
    <div className="flex-1 flex flex-col pt-4 animate-slide-up relative min-h-full px-4 md:px-2 max-w-7xl mx-auto w-full pb-10">
      ...
      <Card className="relative w-full bg-white rounded-2xl md:rounded-3xl flex flex-col md:flex-row items-center justify-between p-6 md:p-8 mb-6 md:mb-8 border border-slate-100 shadow-sm overflow-hidden">
        <div className="absolute top-0 right-0 w-full md:w-1/2 h-full bg-linear-to-l from-amber-500/10 to-transparent pointer-events-none"></div>
